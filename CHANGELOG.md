...
- 修复导入 ChatGPT Team 显示重复账号：去重原先以「account_id + email」为身份，一份失效/失败的旧副本解析不出 account_id 时会退化成另一种 key，永远匹配不上它健康的同一账号副本，导致重复堆积。现改为按「邮箱 + 服务商」去重——Team 成员邮箱各不相同仍各自保留，同一邮箱的副本（含已失效那份）会正确合并。
