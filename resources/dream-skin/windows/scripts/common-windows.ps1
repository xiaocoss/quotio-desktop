...
function Get-DreamSkinCodexAppUserModelId {
  ...
  $familyName = "$($Codex.PackageFamilyName)".Trim()
  if (-not $familyName -or $familyName -notmatch '^[A-Za-z0-9._-]{1,200}$') {
    throw 'The Codex package family name is unavailable or invalid.'
  }
  try {
    $entry = Get-StartApps -ErrorAction Stop |
      Where-Object { "$($_.AppID)" -like "$familyName!*" } |
      Select-Object -First 1
    $appId = "$($entry.AppID)".Trim()
    if ($appId -cmatch ('^' + [regex]::Escape($familyName) + '![A-Za-z0-9._-]{1,200}$')) {
      return $appId
    }
  } catch {}
  if ($Codex.PackageFullName) {
    try {
      $manifest = Get-AppxPackageManifest -Package "$($Codex.PackageFullName)" -ErrorAction Stop
      $application = @($manifest.Package.Applications.Application |
        Where-Object { "$($_.EntryPoint)" -ceq 'Windows.FullTrustApplication' } |
        Select-Object -First 1)
      $applicationId = "$($application.Id)".Trim()
      if ($applicationId -cmatch '^[A-Za-z0-9._-]{1,200}$') {
...
function Initialize-DreamSkinPackagedAppActivator {
  if ($null -ne ('CodexDreamSkin.Interop.PackagedAppLauncher' -as [type])) { return }
  Add-Type -ErrorAction Stop -TypeDefinition @'
using System;
using System.Runtime.InteropServices;

namespace CodexDreamSkin.Interop
{
    [Flags]
...
function Start-DreamSkinCodex {
  ...
  if (-not $Codex.Executable -or -not (Test-Path -LiteralPath "$($Codex.Executable)" -PathType Leaf)) {
  ...
  if ($Codex.PackageFamilyName) {
    $appId = Get-DreamSkinCodexAppUserModelId -Codex $Codex
    try {
      Initialize-DreamSkinPackagedAppActivator
      return [int64][CodexDreamSkin.Interop.PackagedAppLauncher]::Activate($appId, $argumentLine)
    } catch {
      $activationError = $_.Exception.Message
      if (@($Arguments).Count -eq 0) {
        try {
          Start-Process -FilePath "shell:AppsFolder\$appId" -ErrorAction Stop | Out-Null
          return $null
        } catch {
          throw "Codex package activation failed ($appId): $activationError; AppsFolder fallback: $($_.Exception.Message)"
        }
      }
      throw "Codex package activation with launch arguments failed ($appId): $activationError"
    }
  }

  $start = @{
    FilePath = "$($Codex.Executable)"
    ErrorAction = 'Stop'
    PassThru = $true
  }
  if ($argumentLine) { $start.ArgumentList = $argumentLine }
  return (Start-Process @start).Id
}
...
function Stop-DreamSkinRecordedInjector {
  ...
  if ($State.themeDir) {
    $injectorMatches = $injectorMatches -and
