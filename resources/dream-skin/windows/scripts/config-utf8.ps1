...
function Write-DreamSkinBytesAtomically {
  ...
  try {
    ...
    if ([System.IO.File]::Exists($fullPath)) {
      # PowerShell 5.1 and modern pwsh both reject a null backup path here on
      # some .NET builds. A same-directory backup preserves atomic replacement;
      # remove it immediately after the destination swap succeeds.
