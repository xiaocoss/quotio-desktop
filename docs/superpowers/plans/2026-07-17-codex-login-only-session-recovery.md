...
**Goal:** Persist the active Quotio-managed Codex session across app restarts and reliably remove all stale `quotio_bound_login_only` markers when that session ends.

**Architecture:** Extend the existing `~/.codex/quotio-launch-backup.json` with optional non-secret session metadata, then rebuild `AppCore`'s runtime state from that metadata on startup. Centralize marker cleanup in `codex_launch.rs`; lifecycle paths in `lib.rs` call the cleanup operation before restoring Codex state, while `keep_proxy_on_exit=true` preserves the backup, session metadata, and active marker.

**Tech Stack:** Rust, serde/serde_json, Quotio atomic file writes, Cargo tests, TypeScript/Vite build, Tauri release workflow, GitHub CLI.
...
fn legacy_launch_backup_without_session_metadata_still_restores() {
    ...
    assert!(update_launch_session_pid_in(&home, Some(99)).is_err());
...
```powershell
cargo test -p quotio-core launch_backup_round_trips_session_metadata_and_pid -- --nocapture
cargo test -p quotio-core legacy_launch_backup_without_session_metadata_still_restores -- --nocapture
```

Expected: compilation fails because `CodexLaunchSessionState`, the new backup parameter, and metadata helpers do not exist.
...
fn update_launch_session_pid_in(home: &Path, pid: Option<u32>) -> Result<(), String> {
    ...
    let session = backup.session.as_mut()
        .ok_or_else(|| "Codex 启动备份缺少会话信息".to_string())?;
    session.pid = pid;
...
Add a locked public entry point and a directory-scoped implementation. The implementation must read only `.json` files, skip files whose text does not contain `quotio_bound_login_only`, continue processing after individual failures, and return one combined error after attempting every matching file:
...
fn reconcile_login_only_markers_in(
    ...
    let entries = std::fs::read_dir(dir)
        .map_err(|e| format!("读取代理账号目录失败 {}: {e}", dir.display()))?;
    for entry in entries {
        ...
        let key = path.file_stem().and_then(OsStr::to_str).unwrap_or_default();
        if retain_key.is_some_and(|retain| retain.eq_ignore_ascii_case(key)) {
            continue;
        }
        let is_marked = serde_json::from_str::<Value>(&text)
            .ok()
            .and_then(|value| value.get(BOUND_LOGIN_ONLY_FIELD).and_then(Value::as_bool))
            .unwrap_or(false);
        if !is_marked {
            continue;
        }
        match release_bound_account_login_only_in(dir, key) {
            Ok(()) => released += 1,
    ...
    if errors.is_empty() {
        ...
        Err(format!("释放 Codex 登录专用账号失败: {}", errors.join("; ")))
    }
}
...
fn cleanup_managed_codex_disk_state_in(
    ...
    let markers = reconcile_login_only_markers_in(auth_dir, None);
    let restore = if home.join(LAUNCH_BACKUP_FILE).exists() {
        restore_codex_state_from_launch_backup_in(home)
    } else {
        Ok(())
    };
    match (markers, restore) {
        (Ok(count), Ok(())) => Ok(count),
        (Err(marker_error), Ok(())) => Err(marker_error),
        (Ok(_), Err(restore_error)) => Err(restore_error),
        (Err(marker_error), Err(restore_error)) => Err(format!(
            "{marker_error}; 还原 Codex 启动状态失败: {restore_error}"
        )),
    }
}
```
...
Expected: prior single-account behavior remains green and the new two-account cleanup test passes.
...
```powershell
cargo test -p quotio-core recovered_launch_state_restores_profile_account_mode_and_pid -- --nocapture
```
...
fn recovered_codex_runtime(
    ...
    match state {
        Some(state) => (
            Some(codex_launch::CodexSession::new(state.pid, &state.launch_mode)),
...
Add a production startup loader that distinguishes missing, legacy, managed, and parse-error states. Managed startup retains only the persisted active account marker; missing startup clears all markers; legacy and parse-error states preserve markers until explicit cleanup:
    ...
    match codex_launch::load_launch_backup_status_unlocked() {
        Ok(codex_launch::LaunchBackupStatus::Missing) => {
            if let Err(error) = codex_launch::reconcile_login_only_markers_unlocked(None) {
                eprintln!("[startup] cleanup stale Codex markers failed: {error}");
            }
        ...
        Ok(codex_launch::LaunchBackupStatus::Managed(state)) => {
            if let Err(error) = codex_launch::reconcile_login_only_markers_unlocked(
                Some(&state.account_key),
            ) {
                eprintln!("[startup] reconcile Codex markers failed: {error}");
            }
            recovered_codex_runtime(Some(state))
        }
        Err(error) => {
            eprintln!("[startup] read Codex launch state failed: {error}");
            recovered_codex_runtime(None)
        }
    }
}
...
Use `initial_codex_runtime()` in `AppCore::default`. This `cfg(test)` split prevents existing unit tests that call `AppCore::default()` from touching the developer's real `~/.cli-proxy-api` directory. Initialize `codex_session_generation` to `1` when a session was recovered and `0` otherwise.

- [ ] **Step 4: Centralize lifecycle cleanup**

Replace profile-based release with one finish helper. It terminates the process when requested, always attempts marker cleanup and backup restoration through `cleanup_managed_codex_disk_state_unlocked`, clears all in-memory identity even after disk errors, bumps the generation, and reconciles the scheduler:
...
fn finish_codex_session_unlocked(
    ...
    cleanup.map_err(ManagementCoreError::Unavailable)
}
```
...
If PID persistence fails, the snippet kills/closes the just-launched process and returns into the existing launch rollback. That rollback must call `cleanup_managed_codex_disk_state_unlocked()`, clear all three in-memory session fields, bump `codex_session_generation`, reconcile the scheduler, and append any cleanup failure to the original launch error.

At the beginning of `codex_start`, use `self.codex_active()` rather than only `self.codex_session.is_some()`. A recovered same-profile session returns idempotently; a different profile or legacy backup calls `codex_stop_unlocked()` before marking the new account.
...
- **修复开启“退出时保留代理”后，重启 Quotio 再切换第二个 Codex 启动方案会留下多个“Codex 登录专用”账号**：启动备份现在持久化当前方案、账号、启动模式和 PID，重启后可恢复运行状态；停止、切换、启动失败和正常清理时会恢复账号绑定前的启用状态，并清理旧版本遗留标记。
```
